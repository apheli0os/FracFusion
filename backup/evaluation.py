...
class FractionalMaskEvaluator:
    ...
    def simple_adaptation(self, test_images, num_iterations=50):
        ...
        for iteration in range(num_iterations):
            ...
            for name, image in test_images.items():
                ...
                fractional_orders = outputs[1] if isinstance(outputs, (tuple, list)) else outputs
                
                # 设计简单的损失函数，鼓励不同区域有不同的分数阶次
                # 计算图像梯度
                ...
                if len(edge_orders) > 0 and len(smooth_orders) > 0:
                    # 鼓励边缘区域有更正的阶次，平滑区域有更负的阶次
                    edge_loss = -edge_orders.mean()  # 最大化边缘阶次
                    smooth_loss = smooth_orders.abs().mean()  # 最小化平滑区域阶次的绝对值
                    diversity_loss = -fractional_orders.std()  # 鼓励多样性
        ...
        # 修改 comprehensive_evaluation 方法，添加完整的处理效果展示
            ...
            if len(image_files) == 0:
                ...
                for i, img_file in enumerate(image_files[:3]):  # 处理前3张
                    ...
                    try:
                        ...
                        image_tensor, _ = self.load_test_image(img_path, target_size=(128, 128))
    ...
    def parameter_sensitivity_analysis(self):
        ...
        for order_range in order_ranges:
            # 创建新模型（简化版，实际应用中可能需要重新训练）
            print(f"阶次范围 {order_range}: ", end="")
            with torch.no_grad():
                _, orders = self.model(test_image)
                scaled_orders = orders * (order_range[1] - order_range[0]) / 3.0  # 简单缩放
                edge_regions = scaled_orders[scaled_orders > 0.5].numel()
    ...
    # 修改 visualize_processing_effect 方法
    ...
    def visualize_processing_effect(self, image_tensor, save_path=None):
        ...
        with torch.no_grad():
            ...
            if processed_features.shape[1] == image_tensor.shape[1]:
                ...
                # 如果输出是掩膜（如25个通道），则应用到原图像
                if hasattr(self.model, 'apply_fractional_operator'):
                    # 使用分数阶算子处理原图像
                    output_for_viz = self.model.apply_fractional_operator(image_tensor, fractional_orders)
                else:
                    # 简化处理：使用掩膜的前3个通道或平均值
                    if processed_features.shape[1] >= 3:
                        ...
                        # 将掩膜平均后扩展到3个通道
                        mask_avg = processed_features.mean(dim=1, keepdim=True)  # [B, 1, H, W]
                        output_for_viz = mask_avg.expand(-1, 3, -1, -1)  # [B, 3, H, W]
            
            # 转换为numpy
            input_np = image_tensor.squeeze(0).permute(1, 2, 0).cpu().numpy()
            output_np = output_for_viz.squeeze(0).permute(1, 2, 0).cpu().numpy()
            
            # 确保形状匹配
            if output_np.shape != input_np.shape:
                print(f"警告: 输出形状 {output_np.shape} 与输入形状 {input_np.shape} 不匹配")
                if output_np.shape[2] == 1:
                    output_np = np.repeat(output_np, 3, axis=2)
            ...
            # 差异图
            im_diff = axes[0, 2].imshow(diff_np.mean(axis=2), cmap='hot')
            ...
            if True:
                ...
                gray_output = output_for_viz.mean(dim=1, keepdim=True)
                
                sobel_x = torch.tensor([[[[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]]], 
                                    device=self.device, dtype=torch.float32)
                
                input_edges = torch.sqrt(
                    F.conv2d(gray_input, sobel_x, padding=1)**2 + 1e-8
                ).squeeze().cpu().numpy()
                
                axes[1, 1].imshow(input_edges, cmap='gray')
                axes[1, 1].set_title('原始边缘', fontsize=14)
                axes[1, 1].axis('off')
    ...
    def visualize_complete_processing_effect(self, image_tensor, save_path=None):
        ...
        with torch.no_grad():
            # 获取分数阶次
            adaptive_mask, fractional_orders = self.model(image_tensor)
            
            # 应用分数阶处理
            processed_image = self.apply_fractional_processing(image_tensor, fractional_orders)
            ...
            # 边缘检测对比
            gray_input = image_tensor.mean(dim=1, keepdim=True)
            gray_output = processed_image.mean(dim=1, keepdim=True)
            ...
            edge_enhancement = output_edges.mean() / input_edges.mean()
...
# 在evaluation.py的最后，将main_evaluation函数修改为：
def main_evaluation():
    """主评估流程"""
    # 加载模型
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    # 创建分数阶融合上采样模块（直接对RGB做演示）
